...
library TopChain_ {
    ...
    function _vInsert(
        ...
        if (acct != root && chain.nodes[root].ptr > 0 && chain.nodes[root].cat < 2) {
            Node storage n = chain.nodes[acct];
            Node storage r = chain.nodes[root];
        ...
        returns (bool flag)
    {
        ...
        if (n.ptr > 0 && n.cat == 2) {
            ...
            // append new member to tail of topChain
            n.prev = chain.nodes[0].prev;
            n.next = 0;
            chain.nodes[n.prev].next = acct;
            chain.nodes[0].prev = acct;
            ...
            flag = flag && _hInsert(chain, acct, prev, next);
        }
    }
    ...
    function setMaxQtyOfMembers(Chain storage chain, uint64 max) internal {
    ...
    function qtyOfMembers(Chain storage chain) internal view returns (uint16) {
        return uint16(chain.nodes[0].ptr);
    }
    ...
    {
        unchecked {
            return uint16(chain.nodes[0].amt);
        }
    }
        ...
        returns (uint40 group)
    {
        ...
        if (n.ptr == 0) revert("TC.groupNo: acct not member");
        else if (n.cat < 2) group = acct;
        else group = n.ptr;
    }
    ...
    function votesOfGroup(Chain storage chain, uint40 group)
        internal
        view
        returns (uint64 votes)
    {
        Node storage n = chain.nodes[group];

        if (n.ptr > 0) {
            if (n.cat == 1) votes = n.sum;
        } else revert("TC.votesOfGroup: group not exist");
    }
        ...
        returns (uint40[] memory)
    {
        ...
        uint256 i = 0;
        uint40 next = groupNo(chain, acct);
