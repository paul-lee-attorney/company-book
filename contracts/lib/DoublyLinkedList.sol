...
library DoublyLinkedList {
        ...
        returns (bool _success)
    {
        if (self.valid_indexes[_index] == true) {
            ...
            if (item.previous_index == NONE) {
                ...
                self.collection[item.previous_index - 1].next_index = item
                    .next_index;
            }
            ...
            if (item.next_index == NONE) {
                ...
                self.collection[item.next_index - 1].previous_index = item
                    .previous_index;
            }
    ...
    ) public constant returns (address _previous_item) {
        _previous_item = address(
            previous_item(self.data[_collection_index], bytes32(_current_item))
        );
    }
    ...
    function next_item(
        ...
        bytes32 _collection_index,
        address _current_item
    ) public constant returns (address _next_item) {
        _next_item = address(
            next_item(self.data[_collection_index], bytes32(_current_item))
        );
    }
    ...
    ) public constant returns (bytes32 _previous_item) {
        _previous_item = bytes32(
            previous_item(self.data[_collection_index], bytes32(_current_item))
        );
    }
    ...
    function next_item(
        ...
        bytes32 _collection_index,
        bytes32 _current_item
    ) public constant returns (bytes32 _next_item) {
        _next_item = bytes32(
            next_item(self.data[_collection_index], bytes32(_current_item))
        );
    }
