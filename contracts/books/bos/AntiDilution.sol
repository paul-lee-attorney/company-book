...
contract AntiDilution is IAntiDilution, ITerm, BOSSetting, BOMSetting {
    ...
    function addObligor(uint16 class, uint32 obligor)
        external
        onlyAttorney
        onlyMarked(class)
    {
    ...
    function removeObligor(uint16 class, uint32 obligor)
        external
        onlyAttorney
        onlyMarked(class)
    {
    ...
    function giftPar(
        ...
        (, uint64 paid, , , ) = _bos.uint32(shareNumber.ssn());
    ...
    function _isExempted(uint32 price, uint32[] memory consentParties)
        private
        view
        returns (bool)
    {
        ...
        while (cur > 0) {
            ...
            uint32[] memory classMember = _bosCal.membersOfClass(cur);
    ...
    function isExempted(address ia, bytes32 sn) public view returns (bool) {
        ...
        (uint32[] memory consentParties, ) = _bom.getYea(uint256(uint160(ia)));
